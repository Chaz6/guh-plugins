...
//            qCDebug(dcamazonDash()) << "ARP packet doesn't come from an Amazon registered MAC address" << data;
        ...
        if (m_discovering) {
            ...
            descriptor.setParams(ParamList() << Param(dashButtonMacAddressParamTypeId, macAddress));
        ...
        foreach (Device *dev, devices) {
            if (dev->paramValue(dashButtonMacAddressParamTypeId).toString() == macAddress) {
                Event evt(dashButtonPressedEventTypeId, dev->id());
                emit emitEvent(evt);
                break;
            }
        }
